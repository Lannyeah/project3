...
async def test_register(client: AsyncClient):
    response = await client.post(
        "/register", json={"username": "Apollo", "password": "123"}
    )
    await client.post("/register", json={"username": "Adminollo", "password": "456"})
    await client.post("/register", json={"username": "Noownerollo", "password": "789"})
...
async def test_login_n_logout(client: AsyncClient):
    response = await client.post(
        "/login", json={"username": "Apollo", "password": "123"}
    )
...
async def test_invalid_password(client: AsyncClient):
    response = await client.post(
        "/login", json={"username": "Apollo", "password": "000"}
    )
    assert response.status_code == 401
...
async def test_promote_to_admin(admin_client: AsyncClient):
    response = await admin_client.put(
        "/admin/promote-to-admin/2", json={"password": root_pass}
    )
    assert response.status_code == 200
...
async def test_add_category_from_admin(admin_client: AsyncClient):
    response = await admin_client.post("/categories", json={"title": "Бытовые приборы"})
    await admin_client.post("/categories", json={"title": "Надувные игрушки"})
    assert response.status_code == 201
...
async def test_add_category_from_user(authorized_client: AsyncClient):
    response = await authorized_client.post(
        "/categories", json={"title": "Детские игрушки"}
    )
...
async def test_get_category(authorized_client: AsyncClient):
    response = await authorized_client.get("/categories/1")
    data = response.json()
...
async def test_update_category(admin_client: AsyncClient):
    response = await admin_client.put("/categories/1", json={"title": "Просто игрушки"})
    data = response.json()
...
async def test_delete_category_with_equipment(admin_client: AsyncClient):
    response = await admin_client.delete("/categories/2")
    assert response.status_code == 409


@pytest.mark.asyncio
async def test_get_list_of_equipment(authorized_client: AsyncClient):
    response = await authorized_client.get("/equipment")
    data = response.json()
...
async def test_get_equipment(authorized_client: AsyncClient):
    response = await authorized_client.get("/equipment/1")
    data = response.json()
...
async def test_update_equipment(authorized_client: AsyncClient):
    response = await authorized_client.put("/equipment/1", json={"price_per_day": 250})
    data = response.json()
...
async def test_delete_equipment(authorized_client: AsyncClient):
    response = await authorized_client.delete("/equipment/1")
    assert response.status_code == 204
...
async def test_add_photo(authorized_client: AsyncClient):
    response = await authorized_client.post(
        "/equipment/2/photos", files={"file": ("iluha_mad", file, "image/jpeg")}
    )
...
async def test_get_photo(authorized_client: AsyncClient):
    response = await authorized_client.get("/equipment/2/photos/1")
    info = response.json()
    assert info["filename"] == "iluha_mad"
    response = await authorized_client.get("/equipment/2/photos/1/content")
...
async def test_delete_photo(authorized_client: AsyncClient):
    response = await authorized_client.delete("/equipment/2/photos/1")
    assert response.status_code == 204
...
async def test_add_photo_with_bigsize(authorized_client: AsyncClient):
    ...
    response = await authorized_client.post(
        "/equipment/2/photos", files={"file": ("my ego", useless_file, "image/jpeg")}
    )
...
async def test_add_order(authorized_client_2: AsyncClient):
    response = await authorized_client_2.post(
        ...
        json={"start_date": "2025-06-17", "end_date": "2025-06-22"},
    )
    assert response.status_code == 201
...
async def test_add_order_own_equipment(authorized_client: AsyncClient):
    response = await authorized_client.post(
        ...
        json={"start_date": "2025-06-20", "end_date": "2025-07-01"},
    )
    assert response.status_code == 403
...
async def test_get_orders(authorized_client_2: AsyncClient):
    response = await authorized_client_2.get("/orders")
    data = response.json()
    assert isinstance(data, list)
...
async def test_get_orders_from_noncustomer(client: AsyncClient):
    await client.post("/register", json={"username": "Alex", "password": "134"})
    await client.post("/login", json={"username": "Alex", "password": "134"})
...
async def test_delete_order_from_noncustomer(authorized_client: AsyncClient):
    response = await authorized_client.delete("/orders/1")
    assert response.status_code == 403


@pytest.mark.asyncio
async def test_delete_order(authorized_client_2: AsyncClient):
    response = await authorized_client_2.delete("/orders/1")
    assert response.status_code == 204
...
async def test_non_empty_data(client: AsyncClient):
    response = await client.post("/register", json={"username": "  ", "password": "?"})
...
async def test_incomplete_response(client: AsyncClient):
    response = await client.post("/login", json={"password": "134"})
    assert response.status_code == 422
...
async def test_end_date_validate(authorized_client_2: AsyncClient):
    response = await authorized_client_2.post(
        "/orders?equipment_id=3",
        json={"start_date": "2025-06-17", "end-date": "2025-06-12"},
    )
    assert response.status_code == 422
