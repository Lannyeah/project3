...
def non_empty_validator(field_name: str) -> Callable[[type[BaseModel], str], str]:
    ...
    def validate(cls, v: str) -> str:
        ...
        if not v.strip():
            raise ValueError(
                f"{field_title} не может быть пустым или состоять из пробелов"
            )
        return v
...
class EquipmentCreate(BaseModel):
    title: str = Field(
        ...
        title="Наименование",
        description="Должно быть заполнено",
        examples=["Кофеварка", "Принтер"],
    )
    description: str = Field(
        ...,
        title="Описание",
        description="Должно быть заполнено. Максимальный размер - 1000 знаков.",
        examples=["Huevo. Кофеварка бла-бла-бла", "Canyon. Принтер HFDFSVFV-3434"],
    )
    price_per_day: Decimal = Field(
        ...,
        title="Стоимость одного дня аренды",
        description="Должно быть заполнено. Сумма не должна быть отрицательной",
        examples=[100.00, 200.00],
    )
    category_id: int = Field(
        ...,
        title="ID категории",
        description="Должно быть заполнено. Для определения категории обратитесь к get /categories. Если подходящей категории нет, обратитесь в поддержку",
        examples=[1, 2],
    )
...
class EquipmentUpdate(BaseModel):
    title: str | None = Field(
        None, title="Наименование", description="Заполнить, чтобы изменить"
    )
    description: str | None = Field(
        None, title="Описание", description="Заполнить, чтобы изменить"
    )
    price_per_day: Decimal | None = Field(
        None,
        title="Стоимость одного дня аренды",
        description="Заполнить, чтобы изменить",
    )
    category_id: int | None = Field(
        None, title="ID категории", description="Заполнить, чтобы изменить"
    )
...
class PhotoOut(BaseModel):
    ...
    equipment_id: int = Field(
        ...
        description="Оборудование, к которому приложено фото",
    )
...
class CategoryCreate(BaseModel):
    title: str = Field(
        ...
        description="Должно быть заполнено",
        examples=["Электроника", "Бытовая техника"],
    )
...
class CategoryOutFull(BaseModel):
    id: int = Field(..., title="ID категории")
    title: str = Field(..., title="Название категории")
    equipment: list[EquipmentOutbyCategory] = Field(
        ...,
        title="Список оборудования",
        description="Список оборудования, входящего в данную категорию",
    )
...
class OrderCreate(BaseModel):
    start_date: datetime = Field(
        ...
        description="Не может быть после или в один день с датой окончания аренды",
    )
    end_date: datetime = Field(
        ...,
        title="Дата окончания аренды",
        description="Не может быть до или в один день с началом даты аренды",
    )
