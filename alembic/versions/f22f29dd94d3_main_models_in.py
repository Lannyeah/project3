...
def upgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['category_id'], ['categories.id'], name=op.f('fk_equipment_category_id_categories'), ondelete='RESTRICT'),
    sa.ForeignKeyConstraint(['owner_id'], ['users.id'], name=op.f('fk_equipment_owner_id_users'), ondelete='RESTRICT'),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_equipment')),
    sa.UniqueConstraint('title', name=op.f('uq_equipment_title'))
    )
    op.create_index(op.f('ix_equipment_category_id'), 'equipment', ['category_id'], unique=False)
    op.create_index(op.f('ix_equipment_is_available'), 'equipment', ['is_available'], unique=False)
    ...
    sa.Column('total_price', sa.Numeric(precision=10, scale=2), server_default=sa.text('0'), nullable=False),
    sa.ForeignKeyConstraint(['customer_id'], ['users.id'], name=op.f('fk_orders_customer_id_users'), ondelete='RESTRICT'),
    sa.ForeignKeyConstraint(['equipment_id'], ['equipment.id'], name=op.f('fk_orders_equipment_id_equipment'), ondelete='RESTRICT'),
    ...
    sa.Column('equipment_id', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['equipment_id'], ['equipment.id'], name=op.f('fk_photos_equipment_id_equipment'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_photos'))
    )
    op.create_index(op.f('ix_photos_equipment_id'), 'photos', ['equipment_id'], unique=False)
